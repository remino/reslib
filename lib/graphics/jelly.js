...
class Jelly {
	...
	removeEvents() {
		...
		this.pointerProxyEl.removeEventListener(
			'mousemove',
	...
	setUpEvents() {
		// Mouse event handlers
		this.pointerProxyEl.addEventListener(
			'mousemove',
			this.onMouseMove.bind(this),
		)
		this.pointerProxyEl.addEventListener(
	...
	animate() {
		...
		for (let i = 0; i < array.length; i += 3) {
			...
			const noiseValue =
				this.noiseStrength *
				(Math.sin(this.time + x * this.noiseScale) +
					Math.cos(this.time + y * this.noiseScale))
	...
	start() {
		...
		this.pointerProxyEl =
			this.scopeEl.querySelector('[data-jelly=pointer]') ||
...
export default (...scopes) =>
	scopes.flat(Infinity).map((scope) => new Jelly(scope))
